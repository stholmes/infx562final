...
var projection = d3.geoAlbers()
    .scale(9000)
    .translate([-chart_width*2.5, chart_height])
...
Promise.all(promises).then(function(values){
    ...
    d3.select("div.maryland").selectAll('select').on('change', function(d){
        selected_congress =  d3.select("div.maryland").select("#select").property("value")
        redraw_map(svg, values, selected_congress)
     })
 
     d3.select("div.maryland").selectAll('input').on('click', function(d){
     ...
     d3.select("div.maryland").selectAll('path').on('click', function(d){
     ...
     d3.select("div.maryland").select('button.start').on('click', function(d){
     ...
     d3.select("div.maryland").select('button.reset').on('click', function(d){
         selectValue= 0
         redraw_map(svg,values,selected_congress) 
     })
 ...
 function redraw_map(svg, values, index){
     d3.select("div.maryland").selectAll('path').remove()
     svg.selectAll('path')
 ...
 function handleMouseOver(){
     d3.select("div.maryland").select('svg').selectAll('path').select(this)
         .attr('fill', function(d){if (d['properties']['district'] == '3' ) {return '#E86453'}
         else{return'#5FFFF3'}
         })
     return;
 ...
 function handleMouseOut(){
     d3.select("div.maryland").select(this)
     .attr('fill', function(d){if (d['properties']['district'] == '3' ) {return '#E86453'}
     else if(d['properties']['district'] == selectValue){return '#5FFFF3' }
     else{return'#d9d9db'}})
 ...
 function draw_cities(svg){
     if (city_layer == 'on'){
     d3.select("div.maryland").select('div.maryland').selectAll('circle').remove()
         svg.selectAll('circle')
             ...
             .attr('r', 5)
     d3.select("div.maryland").selectAll('.city').remove()
         svg.selectAll('text')
             .data(cities)
             .enter()
             .append('text')
             ...
             .style('font-family', 'sans-serif')
                 ...
                 d3.select("div.maryland").selectAll('circle').remove()
                 d3.select("div.maryland").selectAll('.city').remove()
         }
     return
 }
 ...
 function create_labels(session,index){
     d3.select("div.maryland").selectAll('.label').remove()
     d3.select("div.maryland").select('#facts')
     .append('svg')
     .attr('class', 'label')
     .attr("width", "20")
     .attr("height", "20")
 ...
 function add_capital(svg){
     if (city_layer == 'on'){
         d3.select("div.maryland").selectAll("image").remove()
         var img = svg.select("div.maryland").selectAll("image")
             .data(capital)
             .enter()
             .append("svg:image")
 ...
 function removeClick(){
     d3.select("div.maryland").selectAll('path')
         .attr('id', 'not_clicked')
 }
 ...
 function info(session, index, values){
     ...
     d3.select("div.maryland").select('.district3').remove()
     d3.select("div.maryland").select('#infopane')
         .append('svg')
